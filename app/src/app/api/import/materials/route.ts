...
// Columns: material_code,name,unit,standard_unit_price,lead_time_days,shelf_life_managed,note
// Optional: supplier_name | supplier_code (both resolved by exact Supplier.name match),
...
export async function POST(req: Request) {
  try {
    ...
    for (const [index, r] of rows.entries()) {
      ...
      if (!code) continue;
      const validation = validateMaterialExtensionRow(r);
      if (validation.error) {
      ...
      const data = {
        ...
        shelfLifeManaged: (r.shelf_life_managed ?? "").trim() === "true",
...
function validateMaterialExtensionRow(r: Record<string, string | undefined>): {
  data: {
