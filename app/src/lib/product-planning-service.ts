...
export async function loadMonthlyProductionSchedulePreview({
  ...
  existingPlanStatuses = [...PLANNED_PRODUCTION_PLAN_STATUSES],
  ...
  const [
    ...
    // A系統(DailyReport)は引退し在庫/実績の正は B。未完了予定 (draft/tentative_confirmed/confirmed) とは分けて扱う。
  ...
  // 未完了 (draft/tentative_confirmed/confirmed) の予定数量。完了プランは confirmedReportRows 側で実績として扱う。
